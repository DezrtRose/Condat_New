...
class ApplicationStatus extends Model
{
    ...
    function getApplications($status = 1)
    {
        $applications = CourseApplication::join('clients', 'clients.client_id', '=', 'course_application.client_id')
            ...
            ->select([DB::raw('CONCAT(persons.first_name, " ", persons.last_name) AS fullname'), 'companies.name as company', 'companies.invoice_to_name as invoice_to', 'courses.name', 'intakes.intake_date', 'course_application.course_application_id', 'phones.number', 'emails.email', 'clients.client_id', 'course_application.super_agent_id'])
